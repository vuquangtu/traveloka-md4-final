...
import GifIcon from "../../../components/icon/GifIcon";
...
function ComboFlightTicket(params) {
  const changeSeat = params.changeSeat;
  const disableButton = params.disableButton;
  ...
  const navigate = useNavigate();
    ...
    useEffect(() => {
        if (seat != null) {
            ...
            let startDate = arrStartDate[2] + "-" + arrStartDate[1] + "-" + arrStartDate[0];
            ...
            let floorHour = Math.floor(hour).toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false });
            let minus = Math.floor((hour - floorHour) * 60).toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false });
            ...
            dispatch(changeComBoFlightId(seat.id));
        }
    }, [seat])

    function changeTicket(params) {
        navigate("/combo/flights");
    }
  ...
  return (
    <div className="comboFlightTicket">
      ...
      <div className="comboFlightTicket-right">
        <div className="comboFlightTicket-right-unitPrice">
          ...
          <span className="comboFlightTicket-right-unitPrice-text">
            {" "}
            / khách
          </span>
        </div>
        ...
        <div className="comboFlightTicket-right-change">
          {disableButton === true || changeSeat === true ? null : (
            <div
              onClick={changeTicket}
              className="comboFlightTicket-right-change-button"
            >
              Đổi vé máy bay
            </div>
          )}
...
export default ComboFlightTicket;
